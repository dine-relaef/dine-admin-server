...
func SetupMenuRoutes(router *gin.Engine) {
	MenuGroup := router.Group("/:restaurant_id/menus")
	{
		// Routes for Menus
		MenuGroup.POST("/", controllers.CreateMenu)         // Create a menu
		MenuGroup.GET("/", controllers.GetMenus)            // Get all menus
		...
		menuCategoryGroup := MenuGroup.Group("/:menu_id/categories")
		{
			menuCategoryGroup.POST("/", controllers.CreateMenuCategory)             // Create a menu category for a specific menu
			menuCategoryGroup.GET("/", controllers.GetMenuCategories)              // Get all categories for a specific menu
		...
		menuItemGroup := menuCategoryGroup.Group("/:category_id/items")
		{
			menuItemGroup.POST("/", controllers.CreateMenuItem)             // Create a menu item in a specific category
			menuItemGroup.GET("/", controllers.GetMenuItems)               // Get all items for a specific category
